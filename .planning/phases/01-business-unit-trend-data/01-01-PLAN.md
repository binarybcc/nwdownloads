...
- Create a new API action/endpoint -- embed in the existing overview response.
  ...
  <verify>
  Verify the API response on production:

```bash
# Curl the overview endpoint and check for business_unit_trends key
curl -s 'http://192.168.1.254:8081/api.php?action=overview&compare=previous' | python3 -m json.tool | grep -A 5 'business_unit_trends'
```
...
curl -s 'http://192.168.1.254:8081/api.php?action=overview&compare=previous' | python3 -c "
import json, sys
...
- Remove or modify any existing functionality.
  ...
  <verify>
  Open http://192.168.1.254:8081 in a browser. Open DevTools console and run:
...
2. **Frontend receives and stores data:**
   Open http://192.168.1.254:8081, wait for dashboard to load, then in DevTools console:
