...
e) **Calculate auto-scaled Y-axis from real (non-null) values only:** - Filter out null values to get `realValues` - If no real values exist, use min=0, max=100 - Otherwise: min = floor((min*real - 15% padding) / 10) * 10 (clamped to >= 0), max = ceil((max*real + 15% padding) / 10) * 10 - This ensures the chart Y-axis fits the specific BU's data range, not company totals (requirement CHART-04)
...
2. Verify: each business unit card shows a "12-Week Trend" section with a line chart between the comparison bar and donut chart
3. Verify: chart has blue line with area fill
4. Verify: X-axis shows week labels (W1-W12)
5. Verify: hovering shows tooltip with subscriber count and change indicator
6. Verify: clicking on chart area does NOT open the detail panel
7. Check browser console for JavaScript errors -- should be none
8. Verify: chart resizes properly when browser window is resized
   </verify>
   <done>
