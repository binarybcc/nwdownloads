...
- **SQL migrations** (000*\*.sql, 001*\*.sql, etc.) - Direct schema changes
- **PHP migrations** (YYYYMMDDHHMMSS\_\*.php) - Complex data transformations
...
/usr/local/mariadb10/bin/mysql -uroot circulation_dashboard \
...
```bash
# Pipe migration file through SSH
cat database/migrations/005_create_churn_tables.sql | \
