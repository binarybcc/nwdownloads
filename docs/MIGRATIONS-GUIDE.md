...
# If you see new migration files, run them on the NAS:
ssh nas
/usr/local/mariadb10/bin/mysql -uroot -p -S /run/mysqld/mysqld10.sock circulation_dashboard < database/init/03_add_week_columns.sql
```
...
Run migrations directly on the NAS:
...
# Run migration
/usr/local/mariadb10/bin/mysql -uroot -p -S /run/mysqld/mysqld10.sock circulation_dashboard < /path/to/migration.sql
```

Alternatively, use the migration runner script:
...
```bash
# SSH into NAS, then check what's already been applied
/usr/local/mariadb10/bin/mysql -uroot -p -S /run/mysqld/mysqld10.sock circulation_dashboard -e "DESCRIBE daily_snapshots;"
```

**"Table doesn't exist" error:**

```bash
# SSH into NAS, check database
/usr/local/mariadb10/bin/mysql -uroot -p -S /run/mysqld/mysqld10.sock -e "SHOW DATABASES;"
/usr/local/mariadb10/bin/mysql -uroot -p -S /run/mysqld/mysqld10.sock circulation_dashboard -e "SHOW TABLES;"
```
