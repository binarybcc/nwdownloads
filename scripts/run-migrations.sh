...
# Verify SSH access to NAS
if ! ssh nas "echo ok" &>/dev/null; then
  echo -e "${RED}Error: Cannot connect to NAS via 'ssh nas'${NC}"
  echo "Verify SSH config in ~/.ssh/config"
  exit 1
fi
...
APPLIED_MIGRATIONS=$(ssh nas "/usr/local/mariadb10/bin/mysql -uroot circulation_dashboard -N -e 'SELECT migration_number FROM schema_migrations ORDER BY migration_number;'" 2>/dev/null || echo "")
...
for migration_file in "$PROJECT_ROOT/db_migrations"/*.sql; do
  ...
  if [ -z "$migration_num" ]; then
    echo -e "${YELLOW}Skipping invalid filename: $filename${NC}"
    continue
  fi
  ...
  if [ "$migration_num" = "001" ]; then
    ...
    ssh nas "/usr/local/mariadb10/bin/mysql -uroot circulation_dashboard -e \"INSERT IGNORE INTO schema_migrations (migration_number, migration_name, applied_by) VALUES (1, 'initial_schema', 'system');\"" 2>/dev/null || true
    continue
  fi
  ...
  echo -e "${YELLOW}Applying migration $migration_num: $migration_name${NC}"

  # Run the migration
  if ssh nas "/usr/local/mariadb10/bin/mysql -uroot circulation_dashboard" < "$migration_file"; then
    # Record migration as applied
    ssh nas "/usr/local/mariadb10/bin/mysql -uroot circulation_dashboard -e \"INSERT INTO schema_migrations (migration_number, migration_name, applied_by) VALUES ($migration_num, '$migration_name', '$USER');\""
...
ssh nas "/usr/local/mariadb10/bin/mysql -uroot circulation_dashboard -e 'SELECT migration_number, migration_name, applied_at, applied_by FROM schema_migrations ORDER BY migration_number;'"
