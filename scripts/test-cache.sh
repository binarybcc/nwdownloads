...
curl -s http://192.168.1.254:8081/api/revenue_intelligence.php | jq '.cached, .generated_at, .served_at' 2>/dev/null || echo "Could not parse JSON"
echo ""

sleep 1

echo "2. Second request (should be cache HIT):"
curl -s http://192.168.1.254:8081/api/revenue_intelligence.php | jq '.cached, .cache_generated_at, .served_at' 2>/dev/null || echo "Could not parse JSON"
echo ""

echo "3. Check cache directory:"
ssh nas "ls -lh /tmp/dashboard_cache/" 2>/dev/null | head -10
echo ""

echo "4. Check cache stats:"
ssh nas "/var/packages/PHP8.2/target/usr/local/bin/php82 -r \"
require_once '/volume1/web/circulation/SimpleCache.php';
