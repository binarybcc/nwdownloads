...
def save_to_database(conn, snapshot_date, metrics, vacations):
    ...
    for edition, data in metrics.items():
        ...
        """, (
            snapshot_date, edition, paper_name, business_unit,
            data['total_active'], data['on_vacation'], data['deliverable'],
            data['mail'], data['carrier'], data['digital']
        ))
...
def generate_summary_report(metrics):
    ...
    for edition in ['TJ', 'TA', 'TR', 'LJ', 'WRN', 'FN']:
        ...
        print(f"    Mail:         {data['mail']:5,} ({data['mail']/data['total_active']*100:5.1f}%)")
        print(f"    Digital:      {data['digital']:5,} ({data['digital']/data['total_active']*100:5.1f}%)")
