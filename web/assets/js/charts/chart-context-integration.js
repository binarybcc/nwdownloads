...
async function showSubscriberList(context) {
  ...
  try {
    ...
    const isAggregatedSubscriptionLength =
      chartType === 'subscription_length' &&
    ...
    if (isAggregatedSubscriptionLength) {
      ...
      for (const originalLabel of originalLabels) {
        ...
        if (labelResult.success && labelResult.data && labelResult.data.subscribers) {
          ...
          if (labelResult.data.call_data_as_of) callDataAsOf = labelResult.data.call_data_as_of;
      ...
      callDataAsOf = result.data?.call_data_as_of || null;
    }
    ...
    subscriberPanel.show({
      ...
      data: { subscribers: allSubscribers, call_data_as_of: callDataAsOf },
      // Export metadata
      exportData: {
