...
function normalizeSubscriptionLength(length) {
  ...
  // 1 Year variations
  if (
    normalized.match(/^(12\s*M|1\s*Y|52\s*W|365\s*D)/i) ||
  ...
  // 6 Months variations
  if (normalized.match(/^(6\s*M|26\s*W|182\s*D)/i) || normalized.includes('6 MONTH')) {
    return '6 Months';
  }

  // 3 Months variations
  if (normalized.match(/^(3\s*M|13\s*W|90\s*D)/i) || normalized.includes('3 MONTH')) {
    return '3 Months';
  }

  // 2 Months variations
  if (normalized.match(/^(2\s*M|8\s*W|60\s*D)/i) || normalized.includes('2 MONTH')) {
    return '2 Months';
  }

  // 1 Month variations
  if (normalized.match(/^(1\s*M|4\s*W|30\s*D)/i) || normalized.includes('1 MONTH')) {
    return '1 Month';
  }
...
function populateStateNavigation(businessUnits) {
  ...
  const navHTML = availableBusinessUnits
    .map(unit => {
...
async function loadBusinessUnitData(businessUnit, snapshotDate) {
  ...
  try {
    const response = await fetch(
      `api.php?action=detail_panel&business_unit=${encodeURIComponent(businessUnit)}&snapshot_date=${snapshotDate}`
    );
    ...
    if (result.data.snapshot_date) {
      currentSnapshotDate = window.CircDashboard.detailPanel.currentSnapshotDate =
...
function closeDetailPanel() {
  ...
  if (typeof destroyZoneTrendChart === 'function') {
    destroyZoneTrendChart();
  }
  if (expirationChart) {
...
function renderDetailPanelContent() {
  ...
  if (actualDate !== requestedDate) {
    document.getElementById('detailPanelSubtitle').textContent =
      `Showing data from ${actualDate} (most recent available for week of ${requestedDate})`;
  } else {
  ...
  document.getElementById('detailMail').textContent =
    `${formatNumber(breakdown.mail_delivery)} (${mailPct}%)`;
  document.getElementById('detailDigital').textContent =
    `${formatNumber(breakdown.digital_only)} (${digitalPct}%)`;
  document.getElementById('detailCarrier').textContent =
    `${formatNumber(breakdown.carrier_delivery)} (${carrierPct}%)`;
  ...
  // Load zone trend chart (subscribers by zone over time)
  if (typeof loadZoneTrendChart === 'function') {
    loadZoneTrendChart(currentBusinessUnit);
  }
...
function renderRateDistributionChart(chartData) {
  ...
  rateDistributionChart = window.CircDashboard.detailPanel.charts.rateDistribution = new Chart(
    ctx,
    {
      type: 'bar',
      data: {
...
function renderSubscriptionLengthChart(chartData) {
  ...
  subscriptionLengthChart = window.CircDashboard.detailPanel.charts.subscriptionLength = new Chart(
    ctx,
    {
      type: 'bar',
      data: {
...
function renderComparisonData(comparison) {
  ...
  if (comparison.yoy) {
    ...
    document.getElementById('yoyChange').innerHTML =
      `<span class="${changeColor}">${arrow} ${formatNumber(Math.abs(change))}</span>`;
    document.getElementById('yoyPercent').innerHTML =
      `<span class="${changeColor}">${change >= 0 ? '+' : ''}${percent}%</span>`;
  ...
  if (comparison.previous_week) {
    ...
    document.getElementById('prevWeekChange').innerHTML =
      `<span class="${changeColor}">${arrow} ${formatNumber(Math.abs(change))}</span>`;
    document.getElementById('prevWeekPercent').innerHTML =
      `<span class="${changeColor}">${change >= 0 ? '+' : ''}${percent}%</span>`;
  ...
  if (comparison.trend_direction) {
    ...
    document.getElementById('trendText').innerHTML =
      `<span class="${config.color}">${config.icon} ${config.text}</span>`;
