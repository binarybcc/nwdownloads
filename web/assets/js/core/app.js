...
function initDateNavigation() {
  ...
  const minDateText = minDate.toLocaleDateString('en-US', {
    month: 'short',
    day: 'numeric',
    year: 'numeric',
  });
  const maxDateText = maxDate.toLocaleDateString('en-US', {
    month: 'short',
    day: 'numeric',
    year: 'numeric',
  });
...
function renderEmptyState() {
  ...
  emptyCard.className =
    'col-span-full bg-amber-50 border-2 border-amber-200 rounded-xl p-8 text-center';
...
function renderDashboard() {
  ...
  console.log(
    'renderDashboard called, has_data:',
    dashboardData.has_data,
    'type:',
    typeof dashboardData.has_data
  );
...
function renderKeyMetrics() {
  ...
  const vacPercent =
    current.total_active > 0
      ? ((current.on_vacation / current.total_active) * 100).toFixed(2)
      : '0.00';
...
function renderBusinessUnits() {
  ...
  if (!byUnit) {
    container.innerHTML =
      '<div class="text-center text-gray-500 py-8">No business unit data available</div>';
    return;
  }
  ...
  for (const [unitName, config] of Object.entries(BUSINESS_UNITS)) {
    ...
    html += `
            <div class="paper-card bg-white rounded-xl shadow-sm p-6 border-l-4" style="border-left-color: ${config.color}" data-bu-trend='${JSON.stringify(trendData)}' onclick="openDetailPanel('${unitName}', '${snapshotDate}')">
  ...
  for (const [unitName, _config] of Object.entries(BUSINESS_UNITS)) {
    ...
    businessUnitCharts[unitName] = CircDashboard.state.charts.businessUnits[unitName] = new Chart(
      ctx,
      {
        type: 'doughnut',
        data: {
...
function renderPaperCards() {
  ...
  if (!byPaper) {
    container.innerHTML =
      '<div class="text-center text-gray-500 py-8 col-span-full">No paper data available</div>';
    return;
  }
...
function refreshData() {
  ...
  loadDashboardData().then(() => {
    ...
    button.innerHTML =
      '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg><span>Refresh</span>';
  });
}
...
function exportToExcel() {
  ...
  const unitData = [
    ['Business Unit', 'Papers', 'Total', 'Vacation', 'Deliverable', 'Mail', 'Digital', 'Carrier'],
  ];
  for (const [unit, unitValues] of Object.entries(data.by_business_unit)) {
    ...
    unitData.push([
      ...
      papers,
      unitValues.total,
      unitValues.on_vacation,
      unitValues.deliverable,
      unitValues.mail,
      unitValues.digital,
      unitValues.carrier,
    ]);
  ...
  for (const [code, paperValues] of Object.entries(data.by_edition)) {
    ...
    paperData.push([
      code,
      info?.name || code,
      paperValues.business_unit,
      paperValues.total,
      paperValues.on_vacation,
      paperValues.deliverable,
      paperValues.mail,
      paperValues.digital,
      paperValues.carrier,
    ]);
...
async function exportToPDF() {
  ...
  exportBtn.innerHTML =
    '<div class="loading" style="width:16px;height:16px"></div><span>Generating PDF...</span>';
    ...
    html += `
            <div class="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl shadow p-6 border border-amber-200">
                ...
                <div class="text-sm text-amber-700 mt-2">
                    ${
                      anomalyCount === 1 ? 'anomaly in last 12 weeks' : 'anomalies in last 12 weeks'
                    }
                </div>
                ${
                  latestAnomaly
...
async function toggleBusinessUnitDetails(unitName) {
  ...
  const expandBtn = document.getElementById(
    `expand-${unitName.replace(/\s+/g, '-').toLowerCase()}`
  );
  ...
  if (details.classList.contains('hidden')) {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}?action=business_unit_detail&unit=${encodeURIComponent(unitName)}&date=${currentDate || ''}`
      );
      const result = await response.json();
...
function renderBusinessUnitDetail(unitName, data) {
  ...
  for (const [_paperCode, paperData] of Object.entries(data.paper_details)) {
    const mailPercent =
      paperData.total > 0 ? ((paperData.mail / paperData.total) * 100).toFixed(1) : 0;
    html += `
...
function loadLongestVacations() {
  ...
  fetch(`api.php?action=get_longest_vacations&snapshot_date=${snapshotDate}`)
    ...
    .catch(error => {
      ...
      if (container) {
        container.innerHTML =
          '<div class="text-xs text-gray-400 italic">Unable to load vacation data</div>';
      }
    });
}
