...
function populateExpirationRisk(data) {
  ...
  // Expired
  document.getElementById('expiredCount').textContent = formatNumber(
    totals['Expired']?.subscribers || 0
  );
  document.getElementById('expiredRevenue').textContent = formatCurrency(
    totals['Expired']?.revenue || 0
  );

  // 0-4 Weeks
  document.getElementById('risk04Count').textContent = formatNumber(
    totals['0-4 weeks']?.subscribers || 0
  );
  document.getElementById('risk04Revenue').textContent = formatCurrency(
    totals['0-4 weeks']?.revenue || 0
  );

  // 5-8 Weeks
  document.getElementById('risk58Count').textContent = formatNumber(
    totals['5-8 weeks']?.subscribers || 0
  );
  document.getElementById('risk58Revenue').textContent = formatCurrency(
    totals['5-8 weeks']?.revenue || 0
  );

  // 9-12 Weeks
  document.getElementById('risk912Count').textContent = formatNumber(
    totals['9-12 weeks']?.subscribers || 0
  );
  document.getElementById('risk912Revenue').textContent = formatCurrency(
    totals['9-12 weeks']?.revenue || 0
  );
}
...
function populateLegacyRateAnalysis(data) {
  ...
  // Monthly revenue gap
  document.getElementById('legacyRevenueGap').textContent = formatCurrency(
    totals.monthly_revenue_gap
  );

  // Annual opportunity
  document.getElementById('legacyAnnualOpportunity').textContent = formatCurrency(
    totals.annual_opportunity
  );
}
...
async function showExpirationSubscribers(bucket) {
  ...
  try {
    ...
    const response = await fetch(
      `api/revenue_intelligence.php?action=subscribers&bucket=${encodeURIComponent(bucket)}`
    );
    ...
    const panelData = {
      ...
      data: {
        ...
        call_data_as_of: data.call_data_as_of || null,
      },
    };
...
function populateRevenueByPublication(papers, _snapshotDate) {
  ...
  if (!papers || papers.length === 0) {
    container.innerHTML =
      '<div class="text-center py-12 text-gray-500">No publication data available</div>';
    return;
  }
...
function populateSweetSpotMetrics(metrics) {
  ...
  document.getElementById('annualizedRevenue').textContent = formatCurrency(
    metrics.annualized_revenue || 0
  );
  document.getElementById('minRate').textContent = formatCurrency(metrics.min_rate || 0);
  document.getElementById('maxRate').textContent = formatCurrency(metrics.max_rate || 0);
...
function populateSweetSpotRecommendations(recommendations) {
  ...
  if (!recommendations || recommendations.length === 0) {
    container.innerHTML =
      '<div class="text-center py-4 text-gray-500 italic">No recommendations at this time</div>';
    return;
  }
